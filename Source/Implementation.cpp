...
ReferenceCounterLink::ReferenceCounterLink (DynamicServant_ptr dynamic) :
	reference_counter_ (ObjectFactory_ptr (g_object_factory)->create_reference_counter (dynamic))
{}
...
{
	servant_base_ = ObjectFactory_ptr (g_object_factory)->create_servant (this, dynamic);
}
...
{
	LocalObject_ptr obj = ObjectFactory_ptr (g_object_factory)->create_local_object (base, dynamic);
	object_ = obj;
	return obj;
}
