...
RepId::CheckResult RepId::check (const Char* cur, size_t cur_l, const Char* req, size_t req_l)
{
	...
	Type type_cur = get_type (cur);
	Type type_req = get_type (req);
	if (type_cur != type_req)
		return OTHER_INTERFACE;

	switch (type_cur) {
		case Type::idl: {
			const Char* cur_name = cur + countof (IDL_);
			const Char* cur_end = cur + cur_l;
			const Char* cur_minor = minor_version (version (cur_name, cur_end), cur_end);
			const Char* req_name = req + countof (IDL_);
...
RepId::Type RepId::get_type (const Char* id)
{
	if (is_type (id, IDL_, countof (IDL_) - 1))
		return Type::idl;
	else
		return Type::unknown;
}
